local messageDuration = 3000  -- 3 ثواني

local screenX, screenY = GetActiveScreenResolution()
local boxWidth = 420
local boxHeight = 100
local boxX = (screenX / 2) - 210
local boxY = (screenY / 2) - 100

local textOffsetX = 75
local textOffsetY = 35

local messages = {
    { text = "Press F8 and write camon", size = 26, r = 1, g = 1, b = 1 },
    { text = "SEDK CAMO W SE3DA BYMASO", size = 26, r = 1, g = 1, b = 1 }
}

Citizen.CreateThread(function()
    for _, msg in ipairs(messages) do
        local displayTime = GetGameTimer() + messageDuration

        while GetGameTimer() < displayTime do
            Citizen.Wait(0) -- ضروري عشان ما ينهارش
            Susano.BeginFrame()

            -- خلفية المربع
            Susano.DrawRectFilled(boxX, boxY, boxWidth, boxHeight, 0, 0, 0, 0.8)

            -- الإطار الأبيض الخفيف
            Susano.DrawRect(boxX, boxY, boxWidth, boxHeight, 1, 1, 1, 0.25, 2)

            -- النص داخل المربع
            Susano.DrawText(boxX + textOffsetX, boxY + textOffsetY, msg.text, msg.size, msg.r, msg.g, msg.b, 1, false)
